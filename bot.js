  ...
  forum.getForum({id : process.env.FORUM_NODE_ID}, '',
                 function(_error, _message, body) {
                   body.threads.forEach(function(val) {
                     if ((val.prefix_id === 0) &
                             val.title.toLowerCase().includes('appeal') &&
...
function getUserSteamID(userid, callback) {
  forumDb.query(
      "SELECT provider_key FROM xf_user_connected_account WHERE provider = 'steam' AND user_id = '" +
          userid + "'",
      function(err, result) {
        if (err)
          throw err;
...
function getForumUserBySteamID(steamid, callback) {
  forumDb.query(
      "SELECT user_id FROM xf_user_connected_account WHERE provider = 'steam' AND provider_key = '" +
          steamid + "'",
      function(err, result) {
        if (err)
          throw err;
